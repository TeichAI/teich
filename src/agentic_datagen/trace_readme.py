...
def build_traces_readme(*, pretty_name: str, trace_files: list[Path], tags: list[str], model_id: str | None = None) -> str:
    ...
    if dataset_tools:
        lines.extend(
            [
                ...
                "The same structure is emitted on each loaded example as the `tools` field.",
                "",
            ]
        )
    lines.extend(
    ...
    lines.extend(
        [
            ...
            "",
            "If you want to drive the tokenizer yourself, `load_traces` already returns rows with normalized",
            "`messages` and `tools` fields ready for `tokenizer.apply_chat_template(...)`:",
            "",
            "```python",
